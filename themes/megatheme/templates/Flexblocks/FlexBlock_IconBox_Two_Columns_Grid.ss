<% if $Boxes.count %>
<div class="<% if $HTMLClasses %>{$HTMLClasses}<% else %>block bg-faded p-t-3 p-b-3 overlay-default parallax parallax-v5<% end_if %> block-{$ClassName}" style="background-image: url({$Image.URL});" id="block-{$ID}">
	<div class="container">
		<% if $Heading || $Content %>
		<div class="row">
			<div class="col-md-10 offset-md-1 section-header text-white text-center">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<div class="iconbox iconbox-2-columns">
					<% if $IconClass %>
