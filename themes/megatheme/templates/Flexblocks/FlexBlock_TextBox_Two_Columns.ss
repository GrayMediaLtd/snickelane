<div class="block<% if $HTMLClasses %> {$HTMLClasses}<% end_if %> block-{$ClassName}" id="block-{$ID}">
	<div class="container">
		<div class="row">
		    ...
		    <div class="col-md-6" data-mh="flexbock-textbox">
				<div class="textbox-two-columns">
					<div class="textbox-detail">
						<% if $Title %>
						<h3 class="iconbox-title" data-mh="flexbock-textbox-heading">
							<% if $Link %>
							<a
								href="{$Link}"
						...
						<% if $Content %><div class="typography cleafix flexbock-textbox-content">$Content</div><% end_if %>
					</div>
				</div>
			</div>
			<% end_loop %>
